<?php 
	...
	if (isset($_POST['otmen_za'])){
			$list = $_POST['list'];
			otmen_orders_ojid($list);	
		}
	if (isset($_POST['otpr'])){
			$list = $_POST['list'];
			change_status_otpr($list);	
		}
	if (isset($_POST['dost'])){
			$list = $_POST['list'];
			change_status_dost($list);	
		}
?>
<!DOCTYPE html>
<html lang="en">
		...
		<div class="row" >
		  <div class="col-md-9">
			<div class="panel panel-default">
			  <div class="panel-body">
					<?php 
						$ord = get_tek_orders_manager();
					?>
					<div class="page-header">
						<h3>Текущие заказы (<?= count($ord) ?>):</h3>
					</div>
					<form action="manager.tekorders.php" name="select_all" method="post">
					<table>
							<tr>
							    <td><input type = 'submit' class="btn btn-success" name = 'dost' value = 'Доставлены' />&nbsp;</td>
							    <td><input type = 'submit' class="btn btn-info" name = 'otpr' value = 'Отправлены' />&nbsp;</td>
							    <td><input type = 'submit' class="btn btn-danger" name = 'otmen_za' value = 'Отменить' />&nbsp;</td>
							</tr>
						</table>
						<br>
		  ...
		  <div class="col-md-3">
			<div class="panel panel-default">
				  ...
				  <a href="manager.tekorders.php" class="list-group-item active">Текущие заказы</a>
